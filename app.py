...
# Navigation
pages = ["Manual Entry", "File Upload", "View Data", 
         "Analysis & Visualization", "Predict Protests"]
...
if page == "Predict Protests":
    ...
    if model_file:
        ...
        prediction_type = st.radio("Choose prediction type:", 
                                  ["Single Entry Prediction", "Batch Prediction"])
        ...
        if prediction_type == "Single Entry Prediction":
            ...
            uploaded_data = st.file_uploader("Upload Prediction Data", 
                                            type=["csv", "xlsx"])
            ...
            if uploaded_data:
                try:
                    ...
                    with st.spinner("Making predictions..."):
                        pred_df['Protest_Probability'] = model.predict_proba(pred_df)[:,1]
                        pred_df['Protest_Risk'] = pd.cut(pred_df['Protest_Probability'],
                                                        bins=[0, 0.3, 0.7, 1],
                                                        labels=['Low', 'Medium', 'High'])
                    
                    st.subheader("Prediction Results")
                    st.dataframe(pred_df.sort_values('Protest_Probability', ascending=False))
                    ...
                    st.plotly_chart(px.pie(risk_dist, 
